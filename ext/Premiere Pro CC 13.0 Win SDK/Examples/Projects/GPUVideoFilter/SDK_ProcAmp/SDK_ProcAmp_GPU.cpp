...
#if HAS_METAL
static id<MTLComputePipelineState> sPipelineStateCache[kMaxDevices] = {};
#endif
/*
**
*/
...
public:
		...
		PrGPUFilterInstance* ioInstanceData)
	{
		...
		else if (mDeviceInfo.outDeviceFramework == PrGPUDeviceFramework_Metal)
		{
			...
			mPipelineStateMetal = sPipelineStateCache[mDeviceIndex];
			if (!mPipelineStateMetal)
			{
				prSuiteError result = suiteError_NoError;
				ScopedAutoreleasePool pool;

				//Create a library from source
				NSString *source = [NSString stringWithCString:kSDK_ProcAmp_MetalString encoding:NSUTF8StringEncoding];
				NSError *error = nil;
				...
				if (result == suiteError_NoError)
				{
					...
					mPipelineStateMetal = [device newComputePipelineStateWithFunction:function error:&error];
					result = CheckForMetalError(error);
				}
				sPipelineStateCache[mDeviceIndex] = mPipelineStateMetal;
		...
		PPixHand* outFrame)
	{
		...
		else if (mDeviceInfo.outDeviceFramework == PrGPUDeviceFramework_Metal)
		{
			...
			MTLSize threadsPerGroup = {[mPipelineStateMetal threadExecutionWidth], 16, 1};
			...
			[computeEncoder setComputePipelineState:mPipelineStateMetal];
...
#if HAS_METAL
		ScopedAutoreleasePool pool;
		[sPipelineStateCache[inIndex] release];
		sPipelineStateCache[inIndex] = nil;
#endif
		return suiteError_NoError;
	}
...
#if HAS_METAL
	id<MTLComputePipelineState> mPipelineStateMetal;
#endif
};
